...
export class Task_database {
  ...
  completeTaskInSet = async (completeTaskId: string, setId: string) => {
    ...
    const taskIndex = taskSetList[setId].tasks.findIndex(
      (task: taskRef) => task.taskId == completeTaskId
    );
  ...
  deleteMultiTask = async (deleteTaskIdList: any) => {
    ...
    deleteTaskIdList.forEach((taskref: taskRef) => {
      taskList = taskList.filter(
        (task: ITaskData) => task.id != taskref.taskId
      );
    });
  ...
  editTask = async (updateTask: ITaskData) => {
    ...
    let index = newTaskList.findIndex(
      (task: ITaskData) => task.id == updateTask.id
    );
