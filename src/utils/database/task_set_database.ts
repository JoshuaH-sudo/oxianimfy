...
export class Task_set_database {
  ...
  getSetWithKey = async (setKey: string) => {
    let setList = (await this.store.get("task_set")) ?? this.default_set_list;
    let foundSetId = Object.keys(setList).find(
      (setId: string) => setList[setId].key == setKey
    );
  ...
  findSetsBelongingToTask = async (taskId: string) => {
    ...
    Object.keys(setList).forEach((setId: string) => {
      if (
        setList[setId].tasks.find(
          (taskRef: taskRef) => taskRef.taskId == taskId
        )
      )
        foundSets.push(setId);
    });
    return foundSets;
  };
  ...
  removeTaskFromSet = async (setId: string, taskId: string) => {
    let newTaskSetList =
      (await this.store.get("task_set")) ?? this.default_set_list;
    newTaskSetList[setId].tasks = newTaskSetList[setId].tasks.filter(
      (taskRef: taskRef) => taskRef.taskId != taskId
    );
  ...
  updatedSetTasks = async (
    ...
    newTaskSetList[oldSetId].tasks = newTaskSetList[oldSetId].tasks.filter(
      (taskRef: taskRef) => taskRef.taskId != taskId
    );
